...
typedef NS_ENUM(NSUInteger, TapReachabilityWWANStatus) {
    ...
    TapReachabilityWWANStatus2G =
        2,  ///< Reachable via 2G (GPRS/EDGE)       10~100Kbps
    TapReachabilityWWANStatus3G =
...
@property(nonatomic, readonly)
    SCNetworkReachabilityFlags flags;  ///< Current flags.
@property(nonatomic, readonly)
    TapReachabilityStatus status;      ///< Current status.
@property(nonatomic, readonly)
    TapReachabilityWWANStatus wwanStatus NS_AVAILABLE_IOS(7_0)
        ;            ///< Current WWAN status.
@property(nonatomic, readonly, getter=isReachable)
    BOOL reachable;  ///< Current reachable status.

/// Notify block which will be called on main thread when network changed.
@property(nullable, nonatomic, copy) void (^TapSDKReachabilityNotifyBlock)
    (TapTapSDKReachability *reachability);
...
+ (instancetype)reachabilityForLocalWifi DEPRECATED_MSG_ATTRIBUTE(
...
+ (nullable instancetype)reachabilityWithAddress:
