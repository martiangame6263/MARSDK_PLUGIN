...
@property(nonatomic, strong, nullable) NSString *channel
    __attribute__((deprecated("Use TapTapEventOptions's channel instead")));
...
@property(nonatomic, strong, nullable) NSDictionary *properties
    __attribute__((deprecated("Use TapTapEventOptions's properties instead")));

@property(nonatomic, strong, nullable) NSString *caid;

/// 自定义字段是否能覆盖内置字段
@property(nonatomic, assign) BOOL overrideBuiltInParameters __attribute__((
    deprecated("Use TapTapEventOptions's overrideBuiltInParameters instead")));

/// 是否可以获取 IDFA，默认值为 false
@property(nonatomic, assign) BOOL enableAdvertiserIDCollection;

/// 是否自动上报苹果内购支付成功事件
@property(nonatomic, assign) BOOL enableAutoIAPEvent __attribute__((
    deprecated("Use TapTapEventOptions's enableAutoIAPEvent instead")));
...
+ (void)initWithOptions:(TapTapSdkOptions *)coreOption
           otherOptions:
               (NSArray<id<TapTapSdkBaseOptions>> *_Nullable)otherOptions;
